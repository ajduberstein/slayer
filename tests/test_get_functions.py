...
def test_make_js_get_color():
    ...
    color_js = make_js_get_color(ColorScale(data=FAKE_DATA, variable_name='comitates', palette='BuPu', num_classes=2))
    print(color_js)
    EXPECTATION = ('function (x) { if (0.0 <= x["comitates"] && x["comitates"] < 4.5) '
                   '{ return [224.0, 236.0, 244.0]}  else if (4.5 <= x["comitates"] && x["comitates"] '
                   '< Infinity) { return [180.0, 161.0, 205.5]} }')
    assert check_js_equal(color_js, EXPECTATION)
